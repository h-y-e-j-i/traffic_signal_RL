    ...
    def step(self, actions):
        ...
        if traci.simulation.getTime() <=self._sim_max_time: # 현재 시뮬레이션 시간이 시뮬레이션 최대 시간보다 작다면 = 아직 에피소드가 끝나지 않았다면 
           # 계속 진행           
            ...
            pd.DataFrame(self._ts_results).to_csv("/home/sonic/Desktop/hyeji/sumo-rl-foundation/multi-agent/Icheon/restricted/outputs/signal/time_test_"+str(self._sim_episode)+'.csv', index=False)
