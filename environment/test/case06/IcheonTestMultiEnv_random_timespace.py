...
class SumoIcheonTestMultiEnvironment(MultiAgentEnv):      
    def __init__(self, actions_are_logits, use_gui, sim_max_time, net_file, route_file, algorithm):
        ...
        self._ts_distance["J14"] = 1479.79
        self._ts_distance["J18"] = 2012.125
        self._ts_distance["J22"] = 2318.625
        self._ts_distance["J3"] = 321.975
        self._ts_distance["J5"] = 529.895
        self._ts_distance["J9"] = 1059.385
        ...
        self._veh_result = dict()
        for i in range(1, 11):
            if i == 10 : self._veh_result["veh10"] = list()
            else : self._veh_result["veh0"+str(i)] = list()
    ...
    def step(self, actions):
        ...
        for ts_ID in self._ts_IDs:      
            if ts_ID  == 'J3' and traci.trafficlight.getPhase(ts_ID)==0:
            ...
            elif ts_ID in ['J3', 'J5', 'J22'] and (traci.trafficlight.getPhase(ts_ID) == (self._actions_len[ts_ID]-1)*2-2 or traci.trafficlight.getPhase(ts_ID) == (self._actions_len[ts_ID]-1)*2-1) : 
                self._results["ID"].append(ts_ID + "_G")
                self._results["simulation_time"].append(traci.simulation.getTime())
                self._results["distance"].append(self._ts_distance[ts_ID])
                self._results["class"].append('G')
            elif traci.trafficlight.getPhase(ts_ID) == (self._actions_len[ts_ID]-1)*2:
                self._results["ID"].append(ts_ID + "_G")
                self._results["simulation_time"].append(traci.simulation.getTime())
                self._results["distance"].append(self._ts_distance[ts_ID])
                self._results["class"].append('G')
        ...
        for veh_ID in _veh_IDS:            
            ...
            if veh_ID[-1] == 'U':
                self._veh_result[veh_ID[0:-1]].append(traci.vehicle.getDistance(veh_ID)) 
        ...
        if traci.simulation.getTime() <=self._sim_max_time: # 현재 시뮬레이션 시간이 시뮬레이션 최대 시간보다 작다면 = 아직 에피소드가 끝나지 않았다면 
           # 계속 진행           
            ...
            for cl, index in zip(self._results["class"], range(len(self._results["class"]))):
                if cl == 'U':
                    max_distance = max(self._veh_result[self._results["ID"][index][0:-1]])
                    self._results["distance"][index] = max_distance-self._results["distance"][index]

            pd.DataFrame(self._results).to_csv("/home/sonic/Desktop/hyeji/sumo-rl-foundation/multi-agent/Icheon/random/outputs/time_space_results.csv", index=None)
