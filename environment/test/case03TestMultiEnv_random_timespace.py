...
class SumoCase03TestMultiEnvironment(MultiAgentEnv):      
    def __init__(self, actions_are_logits, use_gui, sim_max_time, net_file, route_file, algorithm):
        ...
        self._observation_space_len = OBS_LEN
    ...
    def step(self, actions):
        ...
        # 차량이 없다면 
            ...
            for cl, index in zip(self._results["class"], range(len(self._results["class"]))):
                if cl == 'U':
                    max_distance = max(self._results["distance"])
                    self._results["distance"][index] = max_distance-self._results["distance"][index]

            pd.DataFrame(self._results).to_csv("/home/sonic/Desktop/hyeji/sumo-rl-foundation/multi-agent/sumo/case03/random/outputs/time_space_results.csv", index=None)
