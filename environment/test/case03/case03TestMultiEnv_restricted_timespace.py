...
class SumoCase03TestMultiEnvironment(MultiAgentEnv):      
    def __init__(self, actions_are_logits, use_gui, sim_max_time, net_file, route_file, algorithm):
        ...
        self._cycle_time = 118

        # 신호등까지의 주행거리
        self._ts_distance = dict()
        self._ts_distance["gneJ00"] = 245
        self._ts_distance["gneJ9"] = 745
    ...
    def reset(self):
        ...
        self._results["ID"] = list()
        self._results["simulation_time"] = list()
        self._results["distance"] = list() # Returns the distance to the starting point like an odometer.
        self._results["class"] = list()
        ...
        for ts_ID in self._ts_IDs:
            ...
            # 신호등별 점등시간 결과 초기화
            for phase_id in range(len(self._phases_info[ts_ID])):
    ...
    def step(self, actions):
        ...
        if self._actions_are_logits:
            actions = {
                k: np.random.choice(ACT_LEN, p=v)                
        ...
        for ts_ID in self._ts_IDs:                      
            ...
            if self._traffic_signal[ts_ID].current_phase_id%2==0 \
                and self._traffic_signal[ts_ID].current_phase_id<=len(self._phases_info[ts_ID])-3 \
                and (self._traffic_signal[ts_ID].ts_run_time>=MIN_TIME and self._traffic_signal[ts_ID].ts_run_time<self._phases_info[ts_ID][self._traffic_signal[ts_ID].current_phase_id]) :
                # 청색불 유지
                if actions[ts_ID] == 0: 
            ...
            # 황색불 or 청색불 최소시간이 지나지 않음 or 주 신호가 진행중
                ...
                # 주 신호
                if self._traffic_signal[ts_ID].current_phase_id == len(self._phases_info[ts_ID])-2:
                    # 청색불이 (총 주기 시간-마지막 황색불) 만큼 진행되지 않았다면
                    if self._traffic_signal[ts_ID].cycle_run_time < self._cycle_time-self._phases_info[ts_ID][-1]:
                        pass
                        # self._traffic_signal[ts_ID].ts_run_time += 1 # 신호 점등시간 누적
                    ...
                    # 끝났다면 
                        ...
                        self._ts_results[ts_ID+'_'+str(self._traffic_signal[ts_ID].current_phase_id)].append(self._traffic_signal[ts_ID].ts_run_time)
                        self._traffic_signal[ts_ID].current_phase_id = (self._traffic_signal[ts_ID].current_phase_id+1)%len(self._phases_info[ts_ID]) # 다음 황색불로 변경
                ...
                elif self._traffic_signal[ts_ID].current_phase_id%2==1 and self._traffic_signal[ts_ID].ts_run_time<self._phases_info[ts_ID][self._traffic_signal[ts_ID].current_phase_id]:
                    # self._traffic_signal[ts_ID].ts_run_time += 1 # 신호 점등시간 누적
                    pass
                ...
                # 청색불이 최대시간을 지났다면 황색불로 변경
                        ...
                        and self._traffic_signal[ts_ID].ts_run_time>=self._phases_info[ts_ID][self._traffic_signal[ts_ID].current_phase_id]:
                ...
                # 황색불이 최소시간을 지났다면 다음 청색불로 변경
                    ...
                    self._ts_results[ts_ID+'_'+str(self._traffic_signal[ts_ID].current_phase_id)].append(self._traffic_signal[ts_ID].ts_run_time)
                    self._traffic_signal[ts_ID].current_phase_id = (self._traffic_signal[ts_ID].current_phase_id+1)%len(self._phases_info[ts_ID]) # 다음 청색불로 변경
                    self._traffic_signal[ts_ID].ts_run_time = 0
        ...
        _veh_IDS = traci.vehicle.getIDList()        

        for ts_ID in self._ts_IDs:                 
            if traci.trafficlight.getPhase(ts_ID) == (len(self._phases_info[ts_ID])-2):
                self._results["ID"].append(ts_ID + "_G")
                self._results["simulation_time"].append(traci.simulation.getTime())
                self._results["distance"].append(self._ts_distance[ts_ID])
                self._results["class"].append('G')
            else :
                self._results["ID"].append(ts_ID + "_R")
                self._results["simulation_time"].append(traci.simulation.getTime())
                self._results["distance"].append(self._ts_distance[ts_ID])
                self._results["class"].append('R')

        for veh_ID in _veh_IDS:            
            self._results["ID"].append(veh_ID)
            self._results["simulation_time"].append(traci.simulation.getTime())
            self._results["distance"].append(traci.vehicle.getDistance(veh_ID)) 
            self._results["class"].append(veh_ID[-1])
        ...
        if traci.simulation.getTime() <=self._sim_max_time: # 현재 시뮬레이션 시간이 시뮬레이션 최대 시간보다 작다면 = 아직 에피소드가 끝나지 않았다면 
           # 계속 진행           
            ...
            pd.DataFrame(self._results).to_csv("/home/sonic/Desktop/hyeji/sumo-rl-foundation/multi-agent/sumo/case03/restricted/outputs/time_space_results.csv", index=None)
    ...
    def _compute_reward(self):        
        ...
        for ts_ID in self._ts_IDs:
            ...
            # 보조 신호등이 청색불이 최소 녹색시간과 최대시간 사이인 경우만 observation을 구한다
            if self._traffic_signal[ts_ID].current_phase_id%2==0 \
                and self._traffic_signal[ts_ID].current_phase_id<=len(self._phases_info[ts_ID])-3 \
                and (self._traffic_signal[ts_ID].ts_run_time>=MIN_TIME and self._traffic_signal[ts_ID].ts_run_time<self._phases_info[ts_ID][self._traffic_signal[ts_ID].current_phase_id]) :
    ...
    # 2. 현재 action + 각 agent에 해당되는 lane의 차량 대기 시간의 합 + 각 agent에 해당되는 lane별 정치 차량 대수
    ...
    # # 3. 현재 action + 각 agent에 해당되는 lane별 정치 차량 대수
    # def _compute_local_observation(self, action):        
    #     state = dict()        
    ...
    #         # observation = action +  각 lane별 정지 차량 대수            
    #         state[ts_ID] = state[ts_ID]+halting_vehicle_number_state
    #     return state