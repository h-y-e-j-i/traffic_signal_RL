...
class SumoIcheonTestMultiEnvironment(MultiAgentEnv):      
    ...
    def step(self, actions):
        ...
        if traci.simulation.getTime() <=self._sim_max_time: # 현재 시뮬레이션 시간이 시뮬레이션 최대 시간보다 작다면 = 아직 에피소드가 끝나지 않았다면 
           # 계속 진행           
            ...
            pd.DataFrame(self._results).to_csv("/home/sonic/Desktop/hyeji/sumo-rl-foundation/multi-agent/Icheon/restricted/outputs/time_space_results.csv", index=None)
    ...
    def _compute_reward(self):        
        ...
        for ts_ID in self._ts_IDs:
            ...
            if self._traffic_signal[ts_ID].current_phase_id%2==0 and self._traffic_signal[ts_ID].current_phase_id<=len(self._phases_info[ts_ID][self._program_ID])-3 and self._traffic_signal[ts_ID].ts_run_time>=MIN_TIME :
